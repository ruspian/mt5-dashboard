...
## Langkah 9 — Setup HTTPS (HANYA jika webapp di-hosting TERPISAH, misal Vercel)

> **Kalau webapp Next.js lo dipasang di VPS yang SAMA dengan bridge ini**
> (lewat PM2 — lihat `webapp/PM2_DEPLOY.md`), **lewati langkah ini**.
> Set `HOST = "127.0.0.1"` di `config.py`, bridge tidak perlu HTTPS atau
> domain sendiri sama sekali — cukup webapp-nya saja yang pakai HTTPS,
> dan bridge diakses lewat `localhost` dari webapp. Ini lebih aman
> karena bridge tidak pernah terekspos ke internet.

Kalau webapp di-hosting terpisah dari VPS ini (misal Vercel), lanjutkan:
...
1. Set `HOST = "0.0.0.0"` di `config.py` (bukan `127.0.0.1`), restart bridge
2. Lo butuh **domain** yang di-pointing ke IP VPS ini (misal beli domain murah di Namecheap ~Rp150rb/tahun, atau kalau cuma buat coba-coba bisa pakai DuckDNS gratis: https://www.duckdns.org)
3. Set DNS domain itu (A record) mengarah ke IP publik VPS lo
4. Download Caddy untuk Windows: https://caddyserver.com/download
5. Taruh `caddy.exe` di `C:\caddy\`
6. Buat file `C:\caddy\Caddyfile` isinya:
   ```
   bridge.domainlo.com {
       reverse_proxy localhost:8765
   }
   ```
7. Buka Command Prompt as Administrator di `C:\caddy`, jalankan:
   ```
   caddy run
   ```
...
8. Sekarang bridge lo bisa diakses aman di `https://bridge.domainlo.com`
9. (Opsional tapi disarankan) Jadikan Caddy juga Windows Service pakai NSSM seperti langkah 8, supaya jalan terus.
